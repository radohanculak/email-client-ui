...
class EmailClient {
  signIn(signInRequest: requests.signInRequest) {
    return axios.post(url + '/auth/sign-in', signInRequest, {
      headers: {
  ...
  signOut() {
    return axios.post(url + '/auth/sign-out', {
      headers: {
        'Content-Type': 'application/json',
      },
    });
  ...
  listEmails(listEmailsRequest: requests.listEmailsRequest) {
    return axios.get(url + '/api/email', {
  ...
  listMailboxes() {
    return axios.get(url + '/api/mailbox', {
      headers: {
        'Content-Type': 'application/json',
      },
    });
  ...
  getEmailInDetail(emailDetailRequest: requests.emailDetailRequest) {
    return axios.get(url + '/api/emailDetail', {
  ...
  downloadAttachment(downloadAttachmentRequest: requests.downloadAttachmentRequest) {
    return axios.get(url + '/api/attachment', {
